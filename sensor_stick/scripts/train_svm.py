...
# Create classifier
estimators = [('pca', PCA()), ('clf', svm.SVC())]
pipe = Pipeline(estimators)
pipe.set_params(clf__random_state=42)
pipe.set_params(clf__kernel='linear')
pipe.set_params(pca__n_components=30)

param_grid = dict(clf__C=[0.001, 0.1, 5e-1, 1, 5, 10, 20, 50, 1000, 10000, 100000])

clf = GridSearchCV(estimator=pipe,
				   param_grid=param_grid,
				   cv=cross_validation.StratifiedKFold(y_train, 5, random_state=42, shuffle=True),
				   scoring=('accuracy'),
				   n_jobs=-1)
